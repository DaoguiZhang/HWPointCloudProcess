...
TextureView::TextureView(bool use_pinhole, std::size_t id, mve::CameraInfo const & camera,
	std::string const & image_file) : id(id), image_file(image_file)
{
...
#if 1
	//内参
	projection[0] = camera.flen; projection[1] = 0.0f; projection[2] = camera.ppoint[0];
	projection[3] = 0.0f; projection[4] = camera.flen; projection[5] = camera.ppoint[1];
	projection[6] = 0.0f; projection[7] = 0.0f; projection[8] = 1.0f;
#endif
...
TextureView::TextureView(std::size_t id, mve::CameraInfo const & camera,
    ...
    camera.fill_viewing_direction(*viewdir);
    camera.fill_world_to_cam(*world_to_cam);
	//camera.fill_intrinsics(c, d, e, cx, cy, coeffs);
}

void TextureView::SetViewParametersFromCamPinholeInfoImg(std::size_t tmp_id, mve::CameraInfo const & camera, std::string const & tmp_image_file)
{
#if 1
	id = tmp_id;
	...
	//std::cout << "the camera flen: " << camera.flen << std::endl;
	//相机的dimension
	width = header.width;
	height = header.height;
	//内参
	...
	projection[3] = 0.0f; projection[4] = camera.flen; projection[5] = camera.ppoint[1];
	projection[6] = 0.0f; projection[7] = 0.0f; projection[8] = 1.0f;
	//std::cout << "proj flen: " << projection[0] << std::endl;
	//std::cout << "camera ppoint is: " << camera.ppoint[0] <<" "<< camera.ppoint[1] << std::endl;
	camera.fill_camera_pos(*pos);
...
#pragma omp parallel for schedule(dynamic)
		for (int j = 0; j < image->height(); j++) {
			if (image->at(i, j, 0) == 0 && image->at(i, j, 1) == 0 && image->at(i, j, 2) == 0) {
				image->at(i, j, 0) = 1;
				image->at(i, j, 1) = 1;
				image->at(i, j, 2) = 1;
			}
...
TextureView::get_face_info(math::Vec3f const & v1, math::Vec3f const & v2,
	...
	/*float dist = get_mesh_camera_dist(v1, v2, v3);
	if (dist < 1.5f)
		face_info->quality = 0;*/
	if (image->at(p1[0], p1[1], 3) == 0 || image->at(p2[0], p2[1], 3) == 0 || image->at(p2[0], p2[1], 3) == 0)
		face_info->quality = 0;
}
