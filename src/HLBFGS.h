...
//! HLBFGS functions
//template<typename T>
//void HLBFGS(int N, int M, double *x,T* ptr, void (T::*EVALFUNC)(int, double*, double*,
...
void HLBFGS(int N, int M, double *x, T* ptr, void (T::*EVALFUNC)(int, double*, double*,
	double*, double*), void EVALFUNC_H(int, double*, double*, double*,
		double*, HESSIAN_MATRIX&), void USER_DEFINED_HLBFGS_UPDATE_H(int, int,
			double*, double*, double*, int, double*, int[]), void(T::*NEWITERATION)(int,
				int, double*, double*, double*, double*), double PARAMETERS[],
	int INFO[])
{
	int T = INFO[6];
...
#pragma omp parallel for private(i)
		...
		gnorm = HLBFGS_DNRM2(N, g);
		INFO[2]++;
		(ptr->*NEWITERATION)(INFO[2], INFO[1], x, &f, g, &gnorm);
