...
void PCAFunctions::PCA(PtCloud<float> &cloud, int k, std::vector<PCAInfo> &pcaInfos)
{
	...
	std::cout << "kd-tree done, begin knn search" << std::endl;
	...
	std::cout << "PCAInfo.size = " << sizeof(PCAInfo) << std::endl;
	std::cout << "pointNum = " << pointNum << std::endl;
	std::cout << "pcaInfos.size = " << sizeof(PCAInfo) * pointNum << " B" << std::endl;
...
#pragma omp parallel for
	for (int i = 0; i<pointNum; ++i)
	{
		...
		float h_mean_x = 0.0, h_mean_y = 0.0, h_mean_z = 0.0;
		for (int j = 0; j < ki; ++j)
		{
			int idx = out_indices[j];
		...
		float h_cov_1 = 0.0, h_cov_2 = 0.0, h_cov_3 = 0.0;
		float h_cov_5 = 0.0, h_cov_6 = 0.0;
		float h_cov_9 = 0.0;
		...
		//int idx = out_indices[3];
		size_t idx = out_indices[3];
		dx = cloud.pts[idx].x - cloud.pts[i].x;
		dy = cloud.pts[idx].y - cloud.pts[i].y;
		dz = cloud.pts[idx].z - cloud.pts[i].z;
		float scaleTemp = sqrt(dx*dx + dy * dy + dz * dz);
		pcaInfos[i].scale = scaleTemp;
		scale += scaleTemp;

		//pcaInfos[i].lambda0 = h_cov_evals.row(2).val[0];
		float t = h_cov_evals.row(0).val[0] + h_cov_evals.row(1).val[0] + h_cov_evals.row(2).val[0] + (rand() % 10 + 1) * MINVALUE;
		pcaInfos[i].lambda0 = h_cov_evals.row(2).val[0] / t;
		pcaInfos[i].normal = h_cov_evectors.row(2).t();
		/*cloud.pts[i].nx = pcaInfos[i].normal(0);
		cloud.pts[i].ny = pcaInfos[i].normal(1);
		cloud.pts[i].nz = pcaInfos[i].normal(2);*/
...
void PCAFunctions::RDPCA(PtCloud<float>& cloud, int k, std::vector<PCAInfo>& pcaInfos)
{
	// removed, bacause it's too slow
}

void PCAFunctions::PCASingle(std::vector<std::vector<float> > &pointData, PCAInfo &pcaInfo)
{
	int i, j;
...
void PCAFunctions::RDPCASingle(std::vector<std::vector<float>>& pointData, PCAInfo & pcaInfo)
{
	int i, j;
	int h0 = 3;
	int k = pointData.size();
	...
	for (int iter = 0; iter < iterTime; ++iter)
	{
		...
		// OD
		std::vector<std::pair<int, float> > ODs(k);
		for (i = 0; i < k; ++i)
		{
			cv::Matx31d ptMat(pointData[i][0], pointData[i][1], pointData[i][2]);
			cv::Matx<float, 1, 1> ODMat = (ptMat - h0_mean).t() * h0_cov_evectors.row(2).t();
			float OD = fabs(ODMat.val[0]);
			ODs[i].first = i;
			ODs[i].second = OD;
		}
		std::sort(ODs.begin(), ODs.end(), [](const std::pair<int, float>& lhs, const std::pair<int, float>& rhs) { return lhs.second < rhs.second; });
	...
	float N = sqrt(pcaInfo.normal.val[0] * pcaInfo.normal.val[0] + pcaInfo.normal.val[1] * pcaInfo.normal.val[1] + pcaInfo.normal.val[2] * pcaInfo.normal.val[2]);
...
void PCAFunctions::MCMD_OutlierRemoval(std::vector<std::vector<float> > &pointData, PCAInfo &pcaInfo)
{
	float a = 1.4826;
	float thRz = 2.5;
	...
	for (int j = 0; j < num; ++j)
	{
		...
		cv::Matx<float, 1, 1> OD_mat = (pt - h_mean).t() * pcaInfo.normal;
	...
	float MAD_OD = a * meadian(abs_diff_ODs) + 1e-6;
	std::vector<float>().swap(abs_diff_ODs);

	// get inlier 
	std::vector<int> idxInlier;
	for (int j = 0; j < num; ++j)
	{
		float Rzi = fabs(ODs[j] - median_OD) / MAD_OD;
		if (Rzi < thRz)
		{
			int idx = pcaInfo.idxAll[j];
...
float PCAFunctions::meadian(std::vector<float> dataset)
{
