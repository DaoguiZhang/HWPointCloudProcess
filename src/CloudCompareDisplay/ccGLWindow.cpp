...
#include <CloudCompareDisplay/ccClipBox.h>
#include <CloudCompareDisplay/ccColorRampShader.h>
#include <CloudCompareDisplay/ccHObjectCaster.h>
#include <CloudCompareDisplay/ccPointCloud.h>
#include <CloudCompareDisplay/ccPolyline.h>
#include <CloudCompareDisplay/ccSphere.h> //for the pivot symbol
#include <CloudCompareDisplay/ccSubMesh.h>

//CCFbo
#include <CloudCompareDisplay/ccFrameBufferObject.h>
#include <CloudCompareDisplay/ccGlFilter.h>
	...
	, m_lockedRotationAxis(0, 0, 1)
{
	...
	//auto-load previous perspective settings
	{
		...
		bool perspectiveView = settings.value(c_ps_perspectiveView, true).toBool();
		...
		else
		{
			...
			if (!m_colorRampShader)
			{
				...
				else
				{
					...
					std::cout << "color ramp shader: " << shaderPath.toStdString() << std::endl;
...
void ccGLWindow::renderNextLODLevel()
{
	...
	{
		ccLog::Print(QString("[renderNextLODLevel] Level %1 - index %2 confirmed").arg(m_currentLODState.level).arg(m_currentLODState.startIndex));
...
void ccGLWindow::draw3D(CC_DRAW_CONTEXT& CONTEXT, RenderingParams& renderingParams)
{
	...
	{
		//test
		//ccLog::Print(QString("[main display]draw the global object"));
		//end test
		m_globalDBRoot->draw(CONTEXT);
	}
	...
	{
		//test
		//ccLog::Print(QString("[main display]draw the window object"));
		//end test
		m_winDBRoot->draw(CONTEXT);
	}
	...
	{
		//test
		//ccLog::Print(QString("[main display]emit lod state"));
		//end test
		emit drawing3D();
	}
	...
	{
		//初始化nextLODState,开始inProgress为false
		renderingParams.nextLODState = LODState();
		if (m_currentLODState.inProgress)
		{
			//当有moreLODPointsAvalable或者更深层次的level的时候，当前的LODState，跳过下一句这时设置inProgress为false
			if (CONTEXT.moreLODPointsAvailable || CONTEXT.higherLODLevelsAvailable)
			{
				...
				{
					//test
					//ccLog::Print(QString("[main display]more lod state level, current lod level is: %1").arg(renderingParams.nextLODState.level));
					//end test
			...
			else
			{
				...
				ccLog::PrintDebug(QString("no more LODPoints available and no more lod level available"));
			}
		}
	}
...
void ccGLWindow::wheelEvent(QWheelEvent* event)
{
	...
	else if (m_interactionFlags & INTERACT_ZOOM_CAMERA)
	{
		...
		float wheelDelta_deg = event->delta() / 3.0f;    // 调节鼠标滚轮缩放速度
