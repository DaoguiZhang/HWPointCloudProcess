...
//Always first
#include "CloudCompareDisplay/ccIncludeGL.h"

#include "CloudCompareDisplay/ccPointCloud.h"

//CCLib
#include <CloudCompareDisplay/GeometricalAnalysisTools.h>
#include <CloudCompareDisplay/ManualSegmentationTools.h>
#include <Common/ReferenceCloud.h>

//local
#include "CloudCompareDisplay/cc2DLabel.h"
#include "CloudCompareDisplay/ccChunk.h"
#include "CloudCompareDisplay/ccColorRampShader.h"
#include "CloudCompareDisplay/ccColorScalesManager.h"
#include "CloudCompareDisplay/ccFastMarchingForNormsDirection.h"
#include "CloudCompareDisplay/ccFrustum.h"
#include "CloudCompareDisplay/ccGBLSensor.h"
#include "CloudCompareDisplay/ccGenericGLDisplay.h"
#include "CloudCompareDisplay/ccGenericMesh.h"
#include "CloudCompareDisplay/ccImage.h"
#include "CloudCompareDisplay/ccKdTree.h"
#include "CloudCompareDisplay/ccMaterial.h"
#include "CloudCompareDisplay/ccMesh.h"
#include "CloudCompareDisplay/ccMinimumSpanningTreeForNormsDirection.h"
#include "CloudCompareDisplay/ccNormalVectors.h"
#include "CloudCompareDisplay/ccOctree.h"
#include "CloudCompareDisplay/ccPointCloudLOD.h"
#include "CloudCompareDisplay/ccPolyline.h"
#include "CloudCompareDisplay/ccProgressDialog.h"
#include "CloudCompareDisplay/ccScalarField.h"
...
void ccPointCloud::drawMeOnly(CC_DRAW_CONTEXT& context)
{
	...
	if (MACRO_Draw3D(context))
	{
				...
				)
			{
				...
				if (!m_lod || !m_lod->isBroken())
				{
					if (!m_lod || m_lod->isNull())
					{
						...
						//test
						ccLog::Print(QString("End init LOD!"));
						//printf("init LOD!\n");
						//end test
					}
					else
					{
						...
						//test
						//ccLog::Print(QString("lod max level: %1").arg(maxLevel));
						//ccLog::Print(QString("lod max is under construction"));
						//end test
						...
						{
							//not yet ready
							ccLog::Print(QString("lod is not ready..."));
							//printf("load not ready...\n");
						...
						else if (context.stereoPassIndex == 0)
						{
							...
							//test
							ccLog::Print(QString("[main display]current lod level: %1").arg(context.currentLODLevel));
							//end test
				...
				if (!toDisplay.indexMap && !skipLoD)
				{
					...
					ccLog::Print(QString("lod is not ready, just skip..."));
		...
		ccLog::Print(QString("[main dispaly]Rendering %1 points starting from index %2 (LoD = %3 / PN = %4)").arg(toDisplay.count).arg(toDisplay.startIndex).arg(toDisplay.indexMap ? "yes" : "no").arg(pushName ? "yes" : "no"));
		...
		//
		//ccLog::Print(QString("[main dispaly]to display point number: %1").arg(toDisplay.count));
		//
		...
		//main display procedure
		{
			...
			{
				//test
				//ccLog::Print(QString("total points num %1").arg(m_pointsVisibility.size()));
				//end test
			...
			{
				//test
				//printf("glParams.showSF: %d\n", glParams.showSF);
				ccLog::Print(QString("[main dispaly] glParams.showSF: %1").arg(glParams.showSF));
				//end test
				assert(m_currentDisplayedScalarField);
			...
			else //no visibility table enabled, no scalar field
			{
				...
				//test
				ccLog::Print(QString("[main display]useVBOs: %1").arg(useVBOs));
				ccLog::Print(QString("[main display]chunck count: %1").arg(chunkCount));
				//printf("useVBOs: %d\n", useVBOs);
				...
				{
					//test
					ccLog::Print(QString("[main display]vbo lod display num: %1").arg(toDisplay.count));
					//printf("vbo lod display num: %d\n", toDisplay.count);
					//end test
					unsigned s = toDisplay.startIndex;
					while (s < toDisplay.endIndex)
					{
				...
				{
					//test
					ccLog::Print(QString("[main display]no indexMap to display num: %1").arg(toDisplay.count));
					ccLog::Print(QString("[main display]no indexMap to display step: %1").arg(toDisplay.decimStep));
					//end test
					for (size_t k = 0; k < chunkCount; ++k)
					{
						size_t chunkSize = ccChunk::Size(k, m_points);
		...
		//test
		//printf("draw points size: %d\n", (int)(toDisplay.count / toDisplay.decimStep));
		//end test
