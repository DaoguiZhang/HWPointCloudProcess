...
namespace HWSFM
{
	...
	*/
	void
		hw_unify_tracks(int view1_tid, int view2_tid,
			HWTrackList* tracks, HWViewportList* viewports)
	{
		...
		HWTrack& track1 = tracks->at(view1_tid);
		HWTrack& track2 = tracks->at(view2_tid);
		...
		track2.features = HWFeatureReferenceList();
	}

	/* ---------------------------------------------------------------- */
	void HWBundleTracks::compute(HWPairwiseMatching const& matching,
			HWViewportList* viewports, HWTrackList* tracks, HWLineTrackList* line_tracks)
	{
		//to do next...
		/* Initialize per-viewport track IDs. -1 */
		for (std::size_t i = 0; i < viewports->size(); ++i)
		{
			HWViewport& viewport = viewports->at(i);
			viewport.track_ids.resize(viewport.features_.positions.size(), -1);
			viewport.lines_track_ids.resize(viewport.lines_segments_.size(), -1);
		}
	...
	//Remove some tracks which do not meet condition
	int	HWBundleTracks::remove_invalid_tracks(HWViewportList* viewports, HWTrackList* tracks)
	{
		//to do next...
		return 1;
	}
