...
namespace HWSFM
{
	...
	void
		HWTrack::invalidate(void)
	{
		this->pos[0] = std::numeric_limits<float>::quiet_NaN();
		this->pos[1] = std::numeric_limits<float>::quiet_NaN();
		this->pos[2] = std::numeric_limits<float>::quiet_NaN();
	}

	void
		HWTrack::remove_view(int view_id)
	{
		for (HWFeatureReferenceList::iterator iter = this->features.begin();
	...
	void
		HWLineTrack::invalidate(void)
	{
		/*std::fill(this->pos.fill), this->pos.end(),
	...
	void
		HWLineTrack::remove_view(int view_id)
	{
		for (HWLineFeatureReferenceList::iterator iter = this->linesfeatures.begin();
			iter != this->linesfeatures.end();)
		{
			if (iter->view_id == view_id)
				iter = this->linesfeatures.erase(iter);
			else
				iter++;
		}
	}
	...
	void
		save_prebundle_data(HWViewportList const& viewports,
			HWPairwiseMatching const& matching, std::ostream& out)
	{
		/* Write signature. */
		...
		{
			HWFeatureSet const& vpf = viewports[i].features_;
			...
			for (std::size_t j = 0; j < vpf.positions.size(); ++j)
				out.write(reinterpret_cast<char const*>(&vpf.positions[j]), sizeof(cv::Vec2f));
			...
			for (std::size_t j = 0; j < vpf.colors.size(); ++j)
				out.write(reinterpret_cast<char const*>(&vpf.colors[j]), sizeof(cv::Vec3b));
		}
		...
		{
			HWTwoViewMatching const& tvr = matching[i];
			int32_t id1 = static_cast<int32_t>(tvr.view_1_id);
			int32_t id2 = static_cast<int32_t>(tvr.view_2_id);
			int32_t num_matches = static_cast<int32_t>(tvr.points_matches.size());
			...
			for (std::size_t j = 0; j < tvr.points_matches.size(); ++j)
			{
				HWCorrespondenceIndex const& c = tvr.points_matches[j];
	...
	void
		load_prebundle_data(std::istream& in, HWViewportList* viewports,
			HWPairwiseMatching* matching)
	{
		...
		{
			HWFeatureSet& vpf = viewports->at(i).features_;
			...
			for (int j = 0; j < num_positions; ++j)
				in.read(reinterpret_cast<char*>(&vpf.positions[j]), sizeof(cv::Vec2f));
			...
			for (int j = 0; j < num_colors; ++j)
				in.read(reinterpret_cast<char*>(&vpf.colors[j]), sizeof(cv::Vec3b));
		}
		...
		for (int32_t i = 0; i < num_pairs; ++i)
		{
			...
			for (int32_t j = 0; j < num_matches; ++j)
			{
				...
				HWCorrespondenceIndex c;
				c.first = static_cast<int>(i1);
				c.second = static_cast<int>(i2);
				tvr.points_matches.push_back(c);
			}
			...
			HWSurveyPointList* survey_points)
	{
		...
		{
			HWSurveyPoint& survey_point = survey_points->at(i);
			for (int j = 0; j < 3; ++j)
		...
		for (std::size_t i = 0; i < num_observations; ++i)
		{
			...
			HWSurveyPoint& survey_point = survey_points->at(point_id);
			...
			float const focal_length)
	{
		...
		double const r2 = std::sqrtf(f.dot(f)) / (focal_length*focal_length);
		double scale = 1.0;
		if (r2 > 0.0)
