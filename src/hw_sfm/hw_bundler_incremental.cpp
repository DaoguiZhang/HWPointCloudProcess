...
/*
all data from hw data is loaded here.
then convert them to HWViewpoints, HWTracklist, and so on

how to implement, to do next...
...
{
	//to do next
	void HWBundleIncremental::initialize(HWViewportList* viewports, HWTrackList* tracks, HWLineTrackList* lines_tracks,
			HWSurveyPointList* survey_points, HWLineSurveyPointList* survey_line_points)
	{
		this->viewports_ = viewports;
		this->tracks_ = tracks;
		this->lines_tracks_ = lines_tracks;
		this->survey_points_ = survey_points;
		this->survey_lines_points_ = survey_line_points;

		if (this->viewports_->empty())
			throw std::invalid_argument("No viewports given");

		/* Check if at least two cameras are initialized. */
		std::size_t num_valid_cameras = 0;
		for (std::size_t i = 0; i < this->viewports_->size(); ++i)
			if (this->viewports_->at(i).pose.is_valid())
				num_valid_cameras += 1;
		...
		{
			HWTrack& track = tracks->at(i);
			track.invalidate();
		}
	}

	void HWBundleIncremental::find_next_views(std::vector<int>* next_views)
	{
		/* Create mapping from valid tracks to view ID. */
		std::vector<std::pair<int, int> > valid_tracks(this->viewports_->size());
		...
		{
			HWTrack const& track = this->tracks_->at(i);
			if (!track.is_valid())
				continue;
			...
			for (std::size_t j = 0; j < track.features.size(); ++j)
			{
				...
				if (this->viewports_->at(view_id).pose.is_valid())
					continue;
	...
	void HWBundleIncremental::triangulate_new_tracks(int min_num_views)
	{
		//compute 3D points
	...
	void HWBundleIncremental::bundle_adjustment_full(void)
	{
		//copy viewports,tracks, survey_points, survey_lines_points to hw_bundle_adjustment
		//then optimize them
		 /* Convert camera to BA data structures. */
		std::vector<BA::Camera> ba_cameras;
		std::vector<int> ba_cameras_mapping(this->viewports_->size(), -1);
		for (std::size_t i = 0; i < this->viewports_->size(); ++i)
		{
			HWViewport const& view = this->viewports_->at(i);
			HWBACameraPose const& pose = view.pose;
			if (!pose.is_valid())
				continue;

			BA::Camera cam;
			cam.focal_length = pose.get_focal_x();	//x = y

			std::copy(pose.t_.data(), pose.t_.data() + 3, cam.translation);
			std::copy(pose.R_.data(), pose.R_.data() + 9, cam.rotation);
		...
		std::vector<int> ba_tracks_mapping(this->tracks_->size(), -1);
		for (std::size_t i = 0; i < this->tracks_->size(); ++i)
		{
			HWTrack const& track = this->tracks_->at(i);
			if (!track.is_valid())
			...
			std::copy(track.pos.val, track.pos.val + 3, point.pos);
			...
			for (std::size_t j = 0; j < track.features.size(); ++j)
			{
				...
				HWViewport const& view = this->viewports_->at(view_id);
				cv::KeyPoint const& f2d = view.features_.positions[feature_id];
				...
				std::copy(f2d_n.val, f2d_n.val + 2, point.pos);
		...
		for (std::size_t i = 0; registered && i < this->survey_points_->size(); ++i)
		{
			HWSurveyPoint const& survey_point = this->survey_points_->at(i);

			/* Add corresponding 3D point to BA. */
			BA::Point3D point;
			//std::copy(survey_point.pos.begin(), survey_point.pos.end(), point.pos);
			//CopyVec3f2Double3(survey_point.pos, point.pos);
			std::copy(survey_point.pos.val, survey_point.pos.val + 3, point.pos);
			...
			{
				HWSurveyObservation const& obs = survey_point.observations[j];
				...
				BA::Observation point;
				std::copy(obs.pos.val, obs.pos.val + 3, point.pos);
				//CopyVec3f2Double3(obs.pos, obs.pos);
				point.camera_id = ba_cameras_mapping[view_id];
		...
		BA::HWBundleAdjustment::HWBAOptions ba_opts;
		BA::HWBundleAdjustment ba(ba_opts);

		//to do next...
		ba.setCameras(&ba_cameras);
		ba.setPoints(&ba_points_3d);
		ba.setObservations(&ba_points_2d);
		ba.optimize();
		ba.print_status();
		...
		for (std::size_t i = 0; i < this->viewports_->size(); ++i)
		{
			...
			HWViewport& view = this->viewports_->at(i);
			HWBACameraPose& pose = view.pose;
			...
			std::copy(cam.translation, cam.translation + 3, pose.t_.data());
			std::copy(cam.rotation, cam.rotation + 9, pose.R_.data());
			std::copy(cam.distortion, cam.distortion + 2, view.radial_distortion);
			pose.set_k_matrix(cam.focal_length, cam.focal_length, 0.0, 0.0);
			ba_cam_counter += 1;
		}
