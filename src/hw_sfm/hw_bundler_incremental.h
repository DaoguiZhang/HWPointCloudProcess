...
#ifndef HWSFM_BUNDLER_INCREMENTAL_HEADER
#define HWSFM_BUNDLER_INCREMENTAL_HEADER
...
namespace HWSFM
{
	...
	public:
		...
		explicit HWBundleIncremental(Options const& options);
		...
		*/
		void initialize(HWViewportList* viewports, HWTrackList* tracks, HWLineTrackList* lines_tracks,
			HWSurveyPointList* survey_points = nullptr, HWLineSurveyPointList* survey_line_points = nullptr);
		...
		//mve::Bundle::Ptr create_bundle(void) const; //to do next...
	...
	private:
		...
		Options opts;
		//store all the bundleadjustments data, such as
	...
	inline
		HWBundleIncremental::Options::Options(void)
		: track_error_threshold_factor(10.0)
		, new_track_error_threshold(0.01)
		, min_triangulation_angle(HWMATH_DEG2RAD(1.0))
	...
	inline HWBundleIncremental::HWBundleIncremental(Options const& options)
		: opts(options)
		, viewports_(nullptr)
		, tracks_(nullptr)
		, lines_tracks_(nullptr)
	{
	}

	inline bool
		HWBundleIncremental::is_initialized(void) const
	{
		return this->viewports_ != nullptr
			&& this->tracks_ != nullptr && this->lines_tracks_;
	}
}
