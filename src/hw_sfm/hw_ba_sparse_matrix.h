...
#ifndef HW_SFM_SPARSE_MATRIX_HEADER
#define HW_SFM_SPARSE_MATRIX_HEADER
...
{
	template <typename T>
	class HWSparseMatrix
	{
	public:
		/** Triplet with row/col index, and the actual value. */
		struct HWTriplet
		{
			HWTriplet(void) = default;
			HWTriplet(std::size_t row, std::size_t col, T const& value);
		...
		/** List of triplets. */
		typedef std::vector<HWTriplet> HWTriplets;

	public:
		HWSparseMatrix(void);
		HWSparseMatrix(std::size_t rows, std::size_t cols);
		void allocate(std::size_t rows, std::size_t cols);
		void reserve(std::size_t num_elements);
		void set_from_triplets(HWTriplets const& triplets);
		void mult_diagonal(T const& factor);
		void cwise_invert(void);
		void column_nonzeros(std::size_t col, HWDenseVector<T>* vector) const;

		HWSparseMatrix transpose(void) const;
		HWSparseMatrix subtract(HWSparseMatrix const& rhs) const;
		HWSparseMatrix multiply(HWSparseMatrix const& rhs) const;
		HWSparseMatrix sequential_multiply(HWSparseMatrix const& rhs) const;
		HWSparseMatrix parallel_multiply(HWSparseMatrix const& rhs) const;
		HWDenseVector<T> multiply(HWDenseVector<T> const& rhs) const;
		HWSparseMatrix diagonal_matrix(void) const;
	...
	template <typename T>
	HWSparseMatrix<T>::HWTriplet::HWTriplet(std::size_t row,
	...
	/* --------------------------------------------------------------- */
	template <typename T>
	HWSparseMatrix<T>::HWSparseMatrix(void)
		: rows(0)
		, cols(0)
	{
	}
	...
	template <typename T>
	HWSparseMatrix<T>::HWSparseMatrix(std::size_t rows, std::size_t cols)
	{
	...
	template <typename T>
	void
		HWSparseMatrix<T>::allocate(std::size_t rows, std::size_t cols)
	{
		this->rows = rows;
		this->cols = cols;
		this->values.clear();
	...
	template <typename T>
	void
		HWSparseMatrix<T>::reserve(std::size_t num_elements)
	{
	...
	template <typename T>
	void
		HWSparseMatrix<T>::set_from_triplets(HWTriplets const& triplets)
	{
		/* Create a temporary transposed matrix */
		HWSparseMatrix<T> transposed(this->cols, this->rows);
	...
	template <typename T>
	HWSparseMatrix<T>
		HWSparseMatrix<T>::transpose(void) const
	{
		HWSparseMatrix ret(this->cols, this->rows);
	...
	template <typename T>
	HWSparseMatrix<T>
		HWSparseMatrix<T>::subtract(HWSparseMatrix const& rhs) const
	{
		if (this->rows != rhs.rows || this->cols != rhs.cols)
			throw std::invalid_argument("Incompatible matrix dimensions");

		HWSparseMatrix ret(this->rows, this->cols);
	...
	template <typename T>
	HWSparseMatrix<T>
		HWSparseMatrix<T>::multiply(HWSparseMatrix const& rhs) const
	{
#ifdef _OPENMP
	...
	template <typename T>
	HWSparseMatrix<T>
		HWSparseMatrix<T>::sequential_multiply(HWSparseMatrix const& rhs) const
	{
		if (this->cols != rhs.rows)
			throw std::invalid_argument("Incompatible matrix dimensions");

		HWSparseMatrix ret(this->rows, rhs.cols);
		ret.reserve(this->num_non_zero() + rhs.num_non_zero());
	...
	template <typename T>
	HWSparseMatrix<T>
		HWSparseMatrix<T>::parallel_multiply(HWSparseMatrix const& rhs) const
	{
		if (this->cols != rhs.rows)
			throw std::invalid_argument("Incompatible matrix dimensions");
...
#pragma omp ordered
	...
	template<typename T>
	HWDenseVector<T>
		HWSparseMatrix<T>::multiply(HWDenseVector<T> const& rhs) const
	{
		if (rhs.size() != this->cols)
			throw std::invalid_argument("Incompatible dimensions");

		HWDenseVector<T> ret(this->rows, T(0));
	...
	template<typename T>
	HWSparseMatrix<T>
		HWSparseMatrix<T>::diagonal_matrix(void) const
	{
		std::size_t const diag_size = std::min(this->rows, this->cols);
		HWSparseMatrix ret(diag_size, diag_size);
	...
	template<typename T>
	void
		HWSparseMatrix<T>::mult_diagonal(T const& factor)
	{
		for (std::size_t i = 0; i < this->outer.size() - 1; ++i)
			for (std::size_t j = this->outer[i]; j < this->outer[i + 1]; ++j)
			{
	...
	template<typename T>
	void
		HWSparseMatrix<T>::cwise_invert(void)
	{
	...
	template<typename T>
	void
		HWSparseMatrix<T>::column_nonzeros(std::size_t col, HWDenseVector<T>* vector) const
	{
	...
	template<typename T>
	inline std::size_t
		HWSparseMatrix<T>::num_non_zero(void) const
	{
	...
	template<typename T>
	inline std::size_t
		HWSparseMatrix<T>::num_rows(void) const
	{
		return this->rows;
	}
	...
	template<typename T>
	inline std::size_t
		HWSparseMatrix<T>::num_cols(void) const
	{
		return this->cols;
	}
	...
	template<typename T>
	void
		HWSparseMatrix<T>::debug(void) const
	{
		std::cout << "HWSparseMatrix ("
