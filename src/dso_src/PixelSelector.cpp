...
void PixelSelector::PrintPixelSuccess()
{
	std::cerr << "pixel ... get it..." << std::endl;
}

void PixelSelector::PrintPixelFrameHessian(FrameHessian* fh)
{
	std::cerr << "pixel ... get it..." << std::endl;
}

void PixelSelector::PrintPixelFrameHessianconst(const FrameHessian* fh)
{
	std::cerr << "pixel ... get it..." << std::endl;
}

void PixelSelector::PrintPixelCalibHessianconst(const CalibHessian* fh)
{
	std::cerr << "PrintPixelCalibHessianconst..." << std::endl;
}

void PixelSelector::PrintPixelMyFrameTestconst(const MyFrameTest* fh)
{
	std::cerr << "PrintPixelMyFrameTestconst const..." << std::endl;
}

void PixelSelector::PrintPixelSuccessconst(const float* myfh)
{
	std::cerr << "pixel ... get it..." << std::endl;
}

void PixelSelector::makeHistsconst(const FrameHessian* fh)
{
...
//			100*numHaveSub/(float)(wG[0]*hG[0]));
	...
	if(plot)
	{
		...
		for(int i=0;i<w*h;i++)
		{
			...
			if(c>255) c=255;
			img.at(i) = Vec3b((int)c,(int)c,(int)c);
		}
...
int PixelSelector::makeMapsNoConst(FrameHessian* fh,
	float* map_out, float density, int recursionsLeft, bool plot, float thFactor)
{
	float numHave = 0;
	float numWant = density;
