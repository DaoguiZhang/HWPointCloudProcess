...
void CoarseTracker::makeCoarseDepthL0(std::vector<FrameHessian*> frameHessians)
{
	...
	for(FrameHessian* fh : frameHessians)
	{
		...
		{
			if(ph->lastResiduals[0].first != 0 && ph->lastResiduals[0].second == ResState::INO)
			{
...
bool CoarseTracker::trackNewestCoarseNoOut(
...
void CoarseDistanceMap::growDistBFS(int bfsNum)
{
	...
	{
		int bfsNum2 = bfsNum;
		//std::swap<Eigen::Vector2i*>(bfsList1,bfsList2);  //error
