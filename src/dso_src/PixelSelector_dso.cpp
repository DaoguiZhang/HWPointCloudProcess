...
PixelSelectorDso::PixelSelectorDso(int w, int h)
{
...
PixelSelectorDso::~PixelSelectorDso()
{
...
void PixelSelectorDso::PrintPixelSuccessDso()
{
...
void PixelSelectorDso::makeHistsDso(const FrameHessian* const fh)
{
	gradHistFrame = fh;
...
int PixelSelectorDso::makeMapsDso(
		...
		float* map_out, float density, int recursionsLeft, bool plot, float thFactor)
{
	...
	//else
	{
		...
		if(fh != gradHistFrame) makeHistsDso(fh);

		// select!
		Eigen::Vector3i n = this->select_dso(fh, map_out,currentPotential, thFactor);
	...
	//				idealPotential);
			...
			return makeMapsDso(fh,map_out, density, recursionsLeft-1, plot,thFactor);
		}
...
Eigen::Vector3i PixelSelectorDso::select_dso(const FrameHessian* const fh,
		float* map_out, int pot, float thFactor)
{
	...
	memset(map_out,0,w*h*sizeof(PixelSelectorStatusDso));
