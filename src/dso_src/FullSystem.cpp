...
//int MyFrameTest::instanceCounter = 0;
...
Vec4 FullSystem::trackNewCoarse(FrameHessian* fh)
{
	...
	/*for (std::size_t i =0; i < outputWrapper.size(); ++i)
	{
		outputWrapper[i]->pushLiveFrame(fh);
	}*/
#endif
	...
	for(unsigned int i=0;i<lastF_2_fh_tries.size();i++)
	{
		...
		bool trackingIsGood = coarseTracker->trackNewestCoarseNoOut(
			fh, lastF_2_fh_this, aff_g2l_this,
			pyrLevelsUsed - 1,
			achievedRes);	// in each level has to be at least as good as the last try.
		tryIterations++;
...
void FullSystem::addActiveFrame( ImageAndExposure* image, int id )
{
	...
	if(!initialized)
	{
		...
		//else if (coarseInitializer->trackFrameNoViews(fh))	// if SNAPPED
		//{
		//	initializeFromInitializer(fh);
		//	lock.unlock();
		//	deliverTrackedFrame(fh, true);
...
void FullSystem::makeKeyFrame( FrameHessian* fh)
{
	...
	for(FrameHessian* fh1 : frameHessians)		// go through all active frames
	{
		...
		for(PointHessian* ph : fh1->pointHessians)
		{
			...
			r->setState(ResState::INO);
			ph->residuals.push_back(r);
			ef->insertResidual(r);
			ph->lastResiduals[1] = ph->lastResiduals[0];
			ph->lastResiduals[0] = std::pair<PointFrameResidual*, ResState>(r, ResState::INO);
			numFwdResAdde+=1;
		}
	}
...
void FullSystem::makeNewTraces(FrameHessian* newFrame, float* gtDepth)
{
	...
	//int recursive_f = 1;
	//bool plotf = false;
	//float thefactorf = 1.0f;
	//int numPointsTotal = pixelSelector->makeMaps(newFrame, selectionMap, setting_desiredImmatureDensity,recursive_f, plotf, thefactorf);
	int numPointsTotal = pixelSelector->makeMapsNoConst(newFrame, selectionMap,setting_desiredImmatureDensity);
	//int numPointsTotal;
	//int w = 100;
	//int h = 100;

#if 0
	FrameHessian* tmpframe = new FrameHessian();
	CalibHessian* tmpcalib = new CalibHessian();
	MyFrameTest* tmpframetest = new MyFrameTest();
	//pixelSelector->makeHists(TMPFrame);
	std::cerr << "aaaa" << std::endl;
	//pixelSelector->PrintPixelSuccess();
	//pixelSelector->PrintPixelFrameHessian(tmpframe);
	const FrameHessian* mytempframe = tmpframe;
	const CalibHessian* mytempcalib = tmpcalib;
	const MyFrameTest* mytemptest = tmpframetest;
	//pixelSelector->PrintPixelFrameHessianconst(mytempframe);
	//pixelSelector->PrintPixelCalibHessianconst(mytempcalib);
	pixelSelector->PrintPixelMyFrameTestconst(mytemptest);

	const float* my;
	pixelSelector->PrintPixelSuccessconst(my);
	//pixelSelector->makeHistsconst(tmpframe);
	delete tmpframe;
#endif
...
void FullSystem::Output3DPoints(std::vector<Eigen::Vector3f>& pnts)
{
