...

    {
        ...
        for (std::int64_t i = 0; i < subgraphs.size(); ++i) {
            ...
            if (success) {
                ...
                //if (settings.keep_unseen_faces) {    // save unseen faces  even no texture
                    #pragma omp critical
        ...
        if (!unseen_faces.empty() && settings.keep_unseen_faces) {
            mve::FloatImage::Ptr image = mve::FloatImage::create(3, 3, 3);
            image->fill(0.0);    // set to black.
        ...
        /*if (!unseen_faces.empty()) {    // save unseen_faces as a new mesh.
            HW::HWMesh unseen_mesh;
            HW::HWMesh unseen_mesh_inv;
            ...
            for (std::size_t i = 0; i < n; i++) {
                ...
                unseen_mesh.AddPoint(point);
                unseen_mesh_inv.AddPoint(point);
            }

            mve::TriangleMesh::NormalList const & normals = mesh->get_vertex_normals();
            if (n != normals.size()) {
                printf("normal nums error!\nmesh.vertices.size() = %d,\nmesh.normals.size() = %d\n", vertices.size(), normals.size());
                n = normals.size();
            }
            for (std::size_t i = 0; i < n; i++) {
                float3 normal;
                ...
                unseen_mesh.AddNormal(normal);
                unseen_mesh_inv.AddNormal(normal);
            }

            const int m = unseen_faces.size();
            for (std::size_t i = 0; i < m; ++i) {
                std::size_t const face_id = unseen_faces[i];
                std::size_t const face_pos = face_id * 3;
                int3 face;
                face.x = mesh_faces[face_pos + 0];
                face.y = mesh_faces[face_pos + 1];
                face.z = mesh_faces[face_pos + 2];
                unseen_mesh.AddFaceIdx(face);
                int3 face_inv;
                face_inv.x = mesh_faces[face_pos + 2];
                face_inv.y = mesh_faces[face_pos + 1];
                face_inv.z = mesh_faces[face_pos + 0];
                unseen_mesh_inv.AddFaceIdx(face_inv);
            }

            const std::string file_name = "D:\\dataset\\DFWG\\UAV_LiDAR_Pano\\unseen_mesh.ply";
            unseen_mesh.SavePly(file_name, HW::PlyFormat::kAscci);
            printf("unseen faces save to %s.\n", file_name.c_str());
            const std::string file_name_inv = "D:\\dataset\\DFWG\\UAV_LiDAR_Pano\\unseen_mesh_inv.ply";
            unseen_mesh_inv.SavePly(file_name_inv, HW::PlyFormat::kAscci);
            printf("unseen faces inv save to %s.\n", file_name_inv.c_str());
        }*/
    }
